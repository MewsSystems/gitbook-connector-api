...
An event management integration sends revenue and payments to the relevant guest’s profile in Mews, pulls live information about rates and availability and allows for the group reservations already sent to Mews to be managed directly from the event management software. The sections below contain guidelines regarding the relevant endpoints used.

### Managing customers

In Mews, billing is managed at the customer’s profile level instead of being charged to a specific room. In order to be able to send the charges to the correct customer, they must have a profile in Mews. In order to retrieve a list of all existing customer profiles created within a certain interval, use the [Get all customers](operations/customers.md#get-all-customers) operation. If a customer does not already exist, they can be added using the [Add customer](operations/customers.md#add-customer) operation and any information can be updated using the [Update customer](operations/customers.md#update-customer) operation. 

### New group reservation 

Any reservation created in the event management software needs to be synced with Mews. A new reservation can be created in Mews using the [Add reservations](operations/reservations.md#add-reservations) operation. The property must be able to create both optional and confirmed reservations
