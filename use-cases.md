...
In order to retrieve all bills and invoices that must be paid and reconciled within the accounting software, the integration should use [Get all closed bills](operations/finance#get-all-closed-bills). To assign the closed bills and invoices to the correct guest journal, the integration should use [Get all customers](operations/customers#get-all-customers) to retrieve the customer `Id`. 
