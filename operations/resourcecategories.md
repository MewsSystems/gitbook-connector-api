...
| `ActivityStates` | array of [Activity state](_objects.md#activity-state) | optional | Whether to return only active, only deleted or both records. The `ActivityState` filter applies to an entity’s own status only. The `IsActive` response property also considers the parent’s status, so some returned records may have `IsActive = false` if their parent is deleted. |
