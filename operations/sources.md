...
Returns all [Sources](#source) assigned to a [Reservation group](reservations.md#reservation-group), there can be multiple sources per reservation group. Note this operation uses [Pagination](../guidelines/pagination.md).
...
| `SourceId` | string | required | Unique identifier of the [Source](#source). |
| `IsPrimary` | bool | required | Specifies the primary source for the [Reservation group](reservations.md#reservation-group). |

## Get all sources

Returns all sources from which reservations are coming from. Note this operation uses [Pagination](../guidelines/pagination.md).
